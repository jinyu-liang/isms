...
public class AuthManager
{
     ...
     */
    public static String getPassword(String username, String resources) throws UserNotFoundException
    {
        String rst = "";
        //        return ServiceLocator.getUserService().getUserByUsername(username).getPassword();
        GgkzUserInfoServiceImpl ggkzUserInfoServiceImpl = SpringContextHolder.getBean("userDetailsService");
        if (ggkzUserInfoServiceImpl != null)
        {
            GgkzUserInfo ggkzUserInfo = new GgkzUserInfo();
            ggkzUserInfo.setUserId(username.toUpperCase());
            //            if (resources != null && !"".equals(resources))
            //            {
            //                try
            //                {
            //                    JSONObject jsonData = JSONObject.fromObject(resources);
            //                    //                    ggkzUserInfo.setMobileSn(jsonData.getString("IMEI"));
            //                    //                    ggkzUserInfo.setMobileId(jsonData.getString("IMSI"));
            //                }
            //                catch (Exception e)
            //                {
            //                    if (log.isErrorEnabled())
            //                    {
            //                        log.error("getPassword异常：{}", e);
            //                    }
            //                }
            //            }
            ggkzUserInfo = ggkzUserInfoServiceImpl.selectUserByEntity(ggkzUserInfo);
            if (ggkzUserInfo != null)
            {
                rst = ggkzUserInfo.getNewPassword();
            }
        }
        return rst;
    }
     ...
     */
    public static AuthToken authenticate(String username, String password, String resources) throws UnauthenticatedException
    {
        ...
        try
        {
            if (!password.equals(getPassword(username, resources)))
            {
                throw new UnauthenticatedException();
            }
        }
     ...
     * @throws UnauthenticatedException if the username and password do not match 
     */
    public static AuthToken authenticate(String username, String token, String digest, String resources) throws UnauthenticatedException
    {
        ...
        try
        {
            String password = getPassword(username, resources);
