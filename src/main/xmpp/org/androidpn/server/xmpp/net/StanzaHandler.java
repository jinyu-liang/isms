...
import com.is.mq.consumer.PmsMsgMobileTopicConsumer;
...
public class StanzaHandler
{
    ...
    private final String     IQ_GET_ROSTER  = "iq_get_roster";
    ...
    public void process(String stanza, XMPPPacketReader reader) throws Exception
    {
        ...
        else if ("presence".equals(tag))
        {
            log.debug("presence...");
            processPresence(doc);
            if (session.getSessionData(IQ_GET_ROSTER) != null)
            {
                session.removeSessionData(IQ_GET_ROSTER);
                ClientSession clientSession = SessionManager.getInstance().getSession(session.getAddress());
                PmsMsgMobileTopicConsumer pmsMsgMobileTopicConsumer = new PmsMsgMobileTopicConsumer();
                if (clientSession != null)
                {
                    clientSession.setJmsSession(pmsMsgMobileTopicConsumer.getMobilesession());
                }
                new Thread(new ReceiveMobileMsg(pmsMsgMobileTopicConsumer, session.getAddress().getNode().toLowerCase())).start();
            }
        }
        else if ("iq".equals(tag))
        {
            log.debug("iq...");
            IQ _iq = getIQ(doc);
            if (IQ.Type.get != _iq.getType())
            {
                Element iq = _iq.getElement();
                if (iq != null)
                {
                    Element query = iq.element("query");
                    if (query != null)
                    {
                        session.setSessionData(IQ_GET_ROSTER, true);
                        ClientSession thatclientSession = SessionManager.getInstance().getSession(query.elementText("username").toLowerCase());
                        if (thatclientSession != null)
                        {
                            ...
                            if (jmsSession != null)
                            {
                                ...
                                {
                                    jmsSession.close();
                                    thatclientSession.setJmsSession(null);
                                }
                                catch (Exception e)
                                {
                                    log.debug("removeSession失败{}", e);
                                }
                            }
                            //                            SessionManager.getInstance().removeSession(thatclientSession);
                        }
                    }
                }
            }
        ...
        {
            log.debug("Unexpected packet tag (not message, iq, presence)" + doc.asXML());
            session.close();
        }
    ...
    private void processPresence(Element doc)
    {
        ...
        {
            log.debug("Ignoring available presence packet of closed session: " + packet);
            return;
        }
    ...
    private void createSession(XmlPullParser xpp) throws XmlPullParserException, IOException
    {
        ...
        if ("jabber:client".equals(namespace))
        {
            ...
            if (session == null)
            {
                ...
                connection.close();
                log.debug("Closing session due to bad_namespace_prefix in stream header: " + namespace);
            }
        }
    }
    ...
    private boolean negotiateTLS()
    {
        if (connection.getTlsPolicy() == Connection.TLSPolicy.disabled)
        {
            ...
            connection.close();
            log.debug("TLS requested by initiator when TLS was never offered" + " by server. Closing connection : " + connection);
            return false;
        }
        ...
        {
            log.debug("Error while negotiating TLS", e);
    ...
    private void startTLS() throws Exception
    {
        ...
        try
        {
            policy = Connection.ClientAuth.valueOf(SystemPropertiesUtil.getStringPropertie("xmpp.client.cert.policy", "disabled"));
        }
        catch (IllegalArgumentException e)
        {
    ...
    private class ReceiveMobileMsg implements Runnable
    {

        private PmsMsgMobileTopicConsumer pmsMobileTopicConsumer;

        private String                    userName;

        ReceiveMobileMsg(PmsMsgMobileTopicConsumer pmsMobileTopicConsumer, String userName)
        {
            this.pmsMobileTopicConsumer = pmsMobileTopicConsumer;
        ...
        public void run()
        {
            if (pmsMobileTopicConsumer != null && userName != null)
            {
                ...
                if (ggkzUserInfoServiceImpl != null)
                {
                    ...
                    ggkzUserInfoQuery.setUserId(userName.toUpperCase());
                    GgkzUserInfo ggkzUserInfo = ggkzUserInfoServiceImpl.selectOneByEntity(ggkzUserInfoQuery);

                    pmsMobileTopicConsumer.receiveMessage(ggkzUserInfo);
                }
            }
        }
