...
public class Connection
{
    ...
    private void notifyCloseListeners()
    {
        if (closeListener != null)
        {
            ...
            {
                log.debug("Error notifying listener: " + closeListener, e);
            }
        }
    }
     ...
     */
    public int deliver(Packet packet)
    {
        int rst = 0;
        ...
        if (!isClosed())
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    boolean ok = ioSession.write(buffer).awaitUninterruptibly(SystemPropertiesUtil.getIntPropertie("connection.ack.timeout", 2000));
                    if (!ok)
                    {
                        log.debug("No ACK was received when sending stanza to: " + this.toString());
                    }
                }
            }
            catch (Exception e)
