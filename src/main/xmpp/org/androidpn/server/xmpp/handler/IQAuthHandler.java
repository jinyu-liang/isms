...
public class IQAuthHandler extends IQHandler
{
    ...
    public IQ handleIQ(IQ packet) throws UnauthorizedException
    {
        ...
        if (session == null)
        {
            log.debug("Session not found for key " + packet.getFrom());
        ...
        try
        {
            ...
            { // set query
                ...
                if (resource != null)
                {
                    String[] dataarr = resource.split("\\?");
                    actionResource = dataarr[0];
                    if (dataarr.length == 2)
                    {
                        dataResource = dataarr[1];
                    }
                }
                ...
                {
                    token = AuthManager.authenticate(username, password, dataResource);
                }
                else if (digest != null && AuthManager.isDigestSupported())
                {
                    token = AuthManager.authenticate(username, session.getStreamID().toString(), digest, dataResource);
                }
                ...
                session.setAuthToken(token, actionResource);
