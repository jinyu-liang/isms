...
<body>
    <div class="page">
        ...
        <div class="pageContent">
            <s:form method="post" action="m-workshop-trans!affirmDelivery"  cssClass="pageForm" onsubmit="return validateCallback(this,$.pdialog.closeCurrentRefresh);" id="%{_}">
                <s:hidden name="entity.transId"></s:hidden>
                <div>
                    <div class="pageFormContent col1" layoutH="56" style="float: left; width: 555px;">
                         ...
                         <s:if test="entity.fileName!=null&&entity.fileName.size()>0">
                            <ul id="${_}myGallery">
                             <s:iterator value="entity.fileName" id="image">
                                <li><img src="<%=basePath+'/'%><s:property value='image'/>"/></li>
                            </s:iterator> 
                            </ul>
                        </s:if>
                        <s:else>
                            <ul id="${_}myGallery">
                                <li><img src="<%=basePath%>/image/pub.jpg" /></li>
                            </ul>
                        </s:else>  
                    </div> 
                </div>
                <div class="formBar">
                    <ul>
                        <sec:authorize ifAllGranted="工地转移-收货">
                        <li>
                                <s:if test='sessionUser.username!=entity.receiver||entity.statusCd!=\"0\"'>
                                    <div class="buttonDisabled">
                                        <div class="buttonContent">
                                            <button type="button" disabled="disabled">确定</button>
                                        </div>
                                    </div>
                                </s:if><s:else>
                                    <div class="button">
                                        <div class="buttonContent">
                                            <button type="button"     onclick="affirmDelivery()">确定</button>
                                        </div>
                                     </div>
                                </s:else>
                        </li>
    ...
    <script type="text/javascript">
		...
		//收货员确认收货
		function affirmDelivery(){
			/* var valradio = $('input:radio[name="entity.statusCd"]:checked').val();
			var memo = $("#entity_memo").val().trim();
			if(valradio=='No'&&memo==''){
				alertMsg.warn("请填写问题原因");
				return false;
			} */
			$("#${_}").submit();
		};
